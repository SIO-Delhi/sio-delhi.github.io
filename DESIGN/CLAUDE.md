...
| Edit member details   | ✓     | —         | —             | Own unit  | —             | Own (pending verification) |
| Approve member edits  | ✓     | —         | —             | Own unit  | —             | —      |
| Set member inactive   | ✓     | ✓         | Own region     | Own unit  | —             | —      |
| Revoke membership     | ✓     | ✓         | —             | —         | —             | —      |
| Search members        | ✓     | ✓         | Own region     | Own unit  | Own unit      | Self   |
| Send messages         | ✓     | ✓         | ✓             | Own unit  | Own unit      | Unit pres, Unit sec, own Regional pres, Zonal sec only |
| Broadcast messages    | ✓     | ✓         | Own region     | Own unit  | Own unit      | —      |
| View own profile      | ✓     | ✓         | ✓             | ✓         | ✓             | ✓      |
| Edit own profile      | ✓     | ✓         | ✓             | ✓         | ✓             | ✓      |

> "Unit pres, Unit sec, own Regional pres, Zonal sec only" means the member can message **only** their unit president, their unit secretary, their own regional president (not all regional presidents), or the zonal secretary. They cannot message other members or regional presidents outside their region.
...
> "Own (pending verification)" means the member can edit their own details, but changes are **not saved immediately**. Instead, a verification request is sent to their unit president. The changes only take effect after the unit president reviews and approves them.
> "Set member inactive" vs "Revoke membership" — these are **two different actions**:
>   - **Set inactive**: Unit Presidents, Regional Presidents, Zonal Secretary, and Admin can set a member as inactive. Before confirming, a **checklist popup** is shown with reasons to verify (e.g. "Not attending programs regularly", "Not giving ayant/dues", "Not participating in unit activities", "Repeated absence from meetings"). The person setting inactive must select at least one reason. The **reason** and **who set them inactive** (`inactivated_by`, `inactive_reason`) are stored and displayed to Admin and Zonal Secretary when viewing the member.
>   - **Revoke membership**: Only Admin and Zonal Secretary can revoke a member's membership (e.g. due to misconduct). This sets their status to `'revoked'` and stores who revoked them, the reason, and when (`revoked_by`, `revoke_reason`, `revoked_at`). Revoked members are **hidden from unit and regional level views** (filtered out of member lists) but remain **visible at admin and zonal level** with a "Membership Revoked" tag. If a unit president or regional president navigates directly to a revoked member's page, they see "MEMBER BANNED" in red. Revoked members can be **restored** by admin/zonal — this is reversible and does NOT delete the member. A separate "Delete Member" action (admin/zonal only) permanently removes the member from the system and Clerk.
> "Search members — Self" means logged-in members can search for themselves (e.g. to view their own record in search results).
...
- User names are stored as **first_name**, **middle_name** (optional), and **last_name** (optional — some members have a single name). The API returns a computed **full_name** for display. In the UI, last name is not required when adding/editing members or in CSV uploads. The database allows `last_name` to be NULL; `portalSetup()` runs a migration to set `last_name` nullable. Username for login is generated as first name (lowercase, alphanumeric) + birth year (e.g. `adnan1998`); duplicates get a suffix (`_2`, `_3`, …).
...
- `/admin/units/manage` - Ability to view, update, and delete units with search and filter functionality, and export to CSV functionality. The table includes a **President** column showing the unit president's name for each unit. **Clicking a unit name** opens the unit detail page (`/admin/units/:unitId`) showing **Unit President** and all **titled members** (e.g. JAC Secretary, Cultural Secretary) directly under the unit name, then active/inactive/migrated and all members with clickable names that go to member profiles. The **member list on unit detail** shows each member's **title** (e.g. Unit President, Unit Secretary, or Campus President / Campus Secretary for campus units) so who is president and who is secretary is visible.
- `/admin/campuses/add` - Ability to add campuses in bulk (or one-by-one) with format examples on upload page.
- `/admin/campuses/manage` - Ability to view, update, and delete campuses. The table includes a **President** column showing the campus president's name. **Clicking a campus name** opens the campus detail page (`/admin/campuses/:campusId`) showing **Campus President** and all **titled members** under the campus name, then member lists with clickable names.
- `/admin/circles/add` - Ability to add circles in bulk (or one-by-one) with format examples on upload page. Circles belong to a region.
- `/admin/circles/manage` - Ability to view, update, and delete circles. **Clicking a circle name** opens the circle detail page (`/admin/circles/:circleId`) showing all **titled members** under the circle name, then member lists with clickable names. Circles do not have a president role.
...
- `/admin/unit-presidents/manage` - Ability to view **region/area** unit president accounts only (campus unit presidents appear under Campus Presidents). Update via inline edit dialogs, delete with confirmation dialogs, search and filter, and export to CSV. The dashboard **Unit Presidents** card shows "X / Y" (units with a president / total region units) with no sublabel; **clicking the card** when there are units without a president navigates to the **Units without president** page.
...
- `/admin/members/:memberId` - **Member Profile View**: Full profile page for any user. Shows hero card (avatar, name, role, status, age bar, title) with a **Delete Member** button (admin only) next to Change Photo / Remove; confirmation dialog explains that the portal account and Clerk login will be removed. On confirm, the user is deleted and the admin is redirected to the members list. Personal information grid and tabbed sections: **Details** (full info), **Messages**, **Performance**, **Migrations**, **Permissions**. Admin has **full edit access**: can modify name, phone, DOB, unit, circle, campus, status, role, avatar, and permission overrides directly from this page. The same profile view is available for zonal-secretaries, regional-presidents, and unit-presidents — clicking their name in the manage table also opens this page.
...
- `/zonal/performance` - Ability to create performance evaluation forms, view responses, and edit forms **at all levels** (zone-wide and unit-scoped). Can create forms with custom fields (MCQ, MSQ, Subjective, Checkbox, Number, Rating). The Zonal Secretary has full access to view, edit, and see responses for all forms across the entire zone.
...
- `/unit/members` - Ability to view and **edit** all members within their unit with search and filter functionality, and export to CSV functionality. Includes performance indicators, title, and activity status for each member. Unit presidents can directly edit member details (name, phone, DOB, etc.) without a verification step. They also receive and approve/reject **verification requests** from members who have edited their own details. **Clicking a member name** opens the member profile view (`/unit/members/:memberId`) with tabs for details, messages, performance, and migration history.
...
- `/member/profile` - Ability to **view** own profile (full name, phone number, unit, title, profile photo, and any additional fields). Members can edit their profile details (name, phone, DOB, etc.), but changes are **not saved immediately** — a **verification request** is sent to their unit president. The unit president must review and approve the changes before they take effect. Profile photo (avatar) and password are editable immediately without verification. The profile overview at the top of the page must display the member's **full name** prominently.
...
- `/member/messages/compose` - Ability to send messages **only** to their unit president, unit secretary, their own regional president, or the zonal secretary. Members cannot message other regional presidents, other members, or anyone else.
...
- Units without president? -> Dashboard stats include `unitsWithoutPresident` (count) and `regionUnitsWithoutPresident` (list of `{ id, name }`). `GET /portal/region-units-without-president` returns that list. The **Unit Presidents** card displays "X / Y" where X = (totalRegionUnits − unitsWithoutPresident) and Y = totalRegionUnits, so the number matches the "Units without president" page (e.g. 17 + 4 = 21). No sublabel text is shown. When the admin clicks the card and there are units without a president, they go to **Units without president** page; otherwise to Unit Presidents manage.
...
- What can members edit on their profile? -> Members can edit their profile details (name, phone, DOB, etc.), but changes require **verification** — a request is sent to their unit president for approval. Only after the unit president approves do the changes take effect. Profile photo (avatar) and password are editable immediately without verification. The profile overview (on dashboard and profile page) must always display the member's full name prominently. The "Account settings & password" link opens a Security-only page (`/member/account`) with Clerk's password change UI — no profile editing, no username change, no "Delete account", no Clerk branding.
- Can unit presidents edit member details? -> Yes. Unit presidents can directly edit the details (name, phone, DOB, etc.) of members within their unit. These edits take effect immediately without a verification step. Unit presidents also receive verification requests from members who have edited their own details and must approve or reject them.
- How does member profile verification work? -> When a member edits their profile details (anything except avatar and password), the changes are stored as a **pending verification request**. The member's unit president sees these pending requests and can approve or reject them. If approved, the changes are applied to the member's record. If rejected, the member is notified and the original data remains unchanged. Avatar and password changes are immediate and do not require verification.
- Who can revoke membership? -> Only Admin and Zonal Secretary can revoke (permanently delete) a member's membership. Unit presidents and regional presidents cannot revoke membership. Revoking deletes the portal user and attempts to delete their Clerk account.
- How does setting a member inactive work? -> Setting inactive is separate from revoking membership. When a Unit President, Regional President, Zonal Secretary, or Admin sets a member as inactive, a **confirmation popup with a checklist** appears. The checklist includes reasons like: "Not attending programs regularly", "Not giving ayant/dues", "Not participating in unit activities", "Repeated absence from meetings". The person must select at least one reason before confirming. The system stores **who** set the member inactive (`inactivated_by` — the user ID of the person who performed the action) and **why** (`inactive_reason` — the selected reasons as a JSON array). When Admin or Zonal Secretary views the member's profile or manage table, they can see who set the member inactive and the reasons. Setting inactive does not delete the member — it only changes their status to inactive. The member can be reactivated later.
- What data is stored when a member is set inactive? -> `portal_users.inactivated_by` (VARCHAR(36), FK to the user who set them inactive), `portal_users.inactive_reason` (JSON array of selected reasons), and `portal_users.inactivated_at` (TIMESTAMP). These fields are cleared when the member is reactivated (set back to active).
- Can members search for themselves? -> Yes. Logged-in members can search and find themselves in search results (e.g. to view their own record).
- Who fills zone-level performance forms? -> Zone-level forms created by the Zonal Secretary or Admin can be filled by anyone depending on the scope of people they have shared with — including members and unit presidents. Unit presidents can only fill (submit responses to) zone-level forms — they cannot edit, delete, or view responses of zone-level forms. The Zonal Secretary has full access to view, edit, and see responses of forms at all levels (zone-wide and unit-scoped).
- Who can members message? -> Members can message **only** their unit president, their unit secretary, their own regional president (the one assigned to their region), and the zonal secretary. They cannot message other regional presidents, other members, or anyone else outside these specific contacts.
...
- Can leaders view a member's full profile? -> Yes. When any leader (Admin, Zonal Secretary, Regional President, Unit President) clicks a member's name in any manage/list table, they navigate to a detailed **Member Profile View** page (`/:role/members/:memberId`). The page shows the member's full profile (hero card with avatar, name, role, age bar, status), personal information grid, and tabbed sections for **Messages** (all sent/received), **Performance** (form responses with expandable answers), **Migrations** (request history), and **Permissions** (admin only — effective permission overrides). **Admin has full edit access**: they can modify all profile fields (name, phone, DOB, unit, circle, campus, status, role, avatar) and permission overrides directly from the profile page. **Admin only**: a **Delete Member** button in the hero opens a confirmation dialog; on confirm, the portal user is deleted and the backend attempts to delete the corresponding Clerk account (best-effort; see below). **Unit President, Regional President, and Zonal Secretary** can set the member **active/inactive** from the profile (scoped to their unit/region/all). All other profile fields are read-only for non-admin roles. The backend exposes three dedicated endpoints: `GET /portal/users/:id/messages`, `GET /portal/users/:id/migrations`, and `GET /portal/users/:id/performance`.
- When is a member's Clerk account deleted? -> When an admin **deletes** a portal user (from Manage Members or from the member profile **Delete Member** button), the backend first deletes the row from `portal_users`, then looks up the user in Clerk by username and calls the Clerk API to delete that account. This is **best-effort**: the portal user is always removed; if Clerk deletion fails (e.g. missing `CLERK_SECRET_KEY`, user not found in Clerk, network error), the API still returns success and may include a `warning` in the response. Actual account deletion (not demotion) is only possible from Manage Members or the member profile; deleting from president/secretary lists **demotes** the user to member instead of deleting.

### Recent implementation changes

- **Last name optional** — Last name is no longer required (single-name members). Frontend: `last_name` is `required: false` in all entity form definitions (Add Members individual/CSV). Backend: `portal_users.last_name` is nullable; `portalCreateUsers` stores null when empty; `portalSetup()` runs `ALTER TABLE portal_users MODIFY COLUMN last_name VARCHAR(128) NULL`. Clerk account creation omits `last_name` when null/empty.
- **Delete Member on profile** — Admin viewing a member profile (`ViewMemberPage`) sees a **Delete Member** button in the hero (next to Change Photo / Remove). Confirmation dialog; on confirm, `DELETE /portal/users/:id` runs, then redirect to members list. Backend attempts Clerk account deletion by username (best-effort).
- **Dashboard Unit Presidents card** — Value is **units with a president / total region units** (e.g. 17/21), not count-of-people/total. No sublabel ("N region unit(s) without president"). Ensures consistency with the "Units without president" page (17 + 4 = 21).
- **Manage lists: President column** — Manage Units shows **President** column (`unit_president_name` from `portalGetUnits`). Manage Campuses shows **President** column (`campus_president_name` from `portalGetCampuses`). Manage Regions already had Regional President. Backend uses subqueries to resolve president name per row.
- **Unit/Campus/Circle detail: President and secretaries under name** — On unit detail, under the unit name: **Unit President:** &lt;name&gt; then **&lt;Title&gt;:** &lt;name&gt; for each titled member (e.g. JAC Secretary, Cultural Secretary). Campus detail: **Campus President:** &lt;name&gt; then titled members. Circle detail: only titled members (no president role). Derived from existing members list in `ViewEntityDetailPage`; no new API.
