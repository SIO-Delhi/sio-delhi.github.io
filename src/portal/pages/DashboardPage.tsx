  ...
  function getStatCards(): DashboardStat[] {
    ...
    if (role === 'admin' || role === 'zonal_secretary' || role === 'regional_president') {
      ...
      cards.push(
        ...
        { label: 'Unit Presidents', value: stats.totalRegionUnits ? `${(stats.totalRegionUnits || 0) - (stats.unitsWithoutPresident ?? 0)} / ${stats.totalRegionUnits}` : stats.totalUnitPresidents, icon: UserCheck, color: 'amber', to: role === 'admin' && (stats.unitsWithoutPresident ?? 0) > 0 ? `${prefix}/unit-presidents/units-without-president` : `${prefix}/unit-presidents${role === 'admin' ? '/manage' : ''}` },
      )
