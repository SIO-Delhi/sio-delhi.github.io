        ...
        <div className="portal-form-stack">
          ...
          <div className="portal-perf-appearance-panel">
            <div className="portal-perf-appearance-header">
              <div>
                <h2>Appearance</h2>
                <p>Customize the uploaded banner, shared footer art, and action color shown on the fill page.</p>
              </div>
            </div>

            <div className="portal-perf-banner-editor">
              <p className="portal-perf-upload-hint">Recommended size: 1200 x 400 pixels (3:1 ratio). Displays at the top of the form.</p>
              <div className={`portal-perf-banner-upload-box ${bannerImage ? 'has-image' : ''}`}>
                {bannerImage ? (
                  <>
                    <img src={bannerImage} alt="Form banner preview" />
                    <div className="portal-perf-banner-floating-actions">
                      <label className="portal-perf-banner-overlay-btn">
                        <Upload size={14} />
                        Change
                        <input type="file" accept="image/*" onChange={handleBannerSelect} disabled={uploadingBanner} className="portal-hidden-input" />
                      </label>
                      <button type="button" onClick={() => setBannerImage(null)} className="portal-perf-banner-remove-btn" aria-label="Remove banner">
                        <Trash2 size={15} />
                      </button>
                    </div>
                  </>
                ) : (
                  <label className="portal-perf-banner-upload-empty">
                    {uploadingBanner ? <Loader2 size={32} className="portal-spin" /> : <Image size={32} />}
                    <span>{uploadingBanner ? 'Uploading...' : 'Click to upload banner image'}</span>
                    <input type="file" accept="image/*" onChange={handleBannerSelect} disabled={uploadingBanner} className="portal-hidden-input" />
                  </label>
                )}
              </div>
            ...
            <div className="portal-perf-color-grid">
              <ColorControl label="Submit Button" value={themePrimaryColor} onChange={setThemePrimaryColor} swatches={['#2563eb', '#0f766e', '#16a34a', '#7c3aed', '#0891b2', '#111827']} />
            ...
            <div className="portal-perf-appearance-live">
              ...
              <button type="button" className="portal-perf-appearance-submit-preview" style={{ background: themePrimaryColor, borderColor: themePrimaryColor }}>
                Submit Response
              </button>
