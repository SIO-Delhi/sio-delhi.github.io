...
export function BulkAddPage({ entity }: BulkAddPageProps) {
  ...
  const handleUpload = useCallback(async (rows: Record<string, string>[]) => {
    ...
    const [units, circles] = await Promise.all([api.fetchUnits(), api.fetchCircles()])
    ...
    const users = rows.map((r, idx) => {
      const user: { first_name: string; middle_name?: string; last_name: string; phone: string; password?: string; date_of_birth?: string; role: PortalRole; unit_id?: string; circle_id?: string } = {
